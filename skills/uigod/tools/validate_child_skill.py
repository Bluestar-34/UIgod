#!/usr/bin/env python3
"""Validate a UIgod style package against the lean contract."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate a UIgod style package folder.")
    parser.add_argument("skill_dir", help="Path to style package folder")
    ...
    parser.add_argument("--managed-style", action="store_true", help="Deprecated compatibility flag; managed styles also use SKILL.md")
    ...
    required_paths = [instruction_file, *BASE_REQUIRED_PATHS]
    ...
    instruction_md = root / instruction_file
    ...
    if instruction_md.exists():
        text = read_text(instruction_md)
        if not has_frontmatter(text):
            errors.append(f"{instruction_file} missing YAML frontmatter")
        if "DESIGN.md" not in text:
            warnings.append(f"{instruction_file} does not mention DESIGN.md")
        if "source-observations.md" not in text:
            warnings.append(f"{instruction_file} does not mention source-observations.md")
        if PLACEHOLDER_RE.search(text) and not args.allow_template:
            errors.append(f"{instruction_file} still contains placeholders")
